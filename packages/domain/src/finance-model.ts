...
export type FinanceBalanceSettingItem = {
  name: string;
  amount: number;
};

export type FinanceBalanceSettings = {
  fundAccounts: FinanceBalanceSettingItem[];
  liabilities: FinanceBalanceSettingItem[];
  investments: FinanceBalanceSettingItem[];
};
...
export function createCaptureDraft(
  ...
  return {
    ...
    targetAccount:
      direction === "transfer"
        ? getAlternateRegistryOption(
            config.registries.transfer,
            config.registries.transfer[0] ?? ""
          )
        : "",
    note: "",
    customRegistryName: "",
...
export function ensureCaptureDraft(
  ...
  const channelAccount = ensureOption(
    draft.channelAccount,
    config.registries[direction]
  );
  const targetAccount =
    direction === "transfer"
...
export function addRegistryItem(
  ...
  if (!trimmed || registries[direction].includes(trimmed)) {
    return registries;
  }
...
export function createLedgerEntryFromDraft(
  draft: FinanceCaptureDraft,
  periodLabel: string,
  date = new Date()
  ...
  return {
    id: `entry-${normalizedDate.isoDate}-${Date.now()}`,
    periodLabel,
    occurredOn: normalizedDate.isoDate,
    displayDate: normalizedDate.monthDay,
    direction: draft.direction,
    amount,
    category: draft.category,
    channelAccount: draft.channelAccount,
    targetAccount: draft.direction === "transfer" ? draft.targetAccount : "",
    note: draft.note.trim() || draft.category
  };
}

export function createRecentFlowFromEntry(entry: FinanceLedgerEntry): FinanceRecentFlow {
  const description =
    entry.direction === "transfer"
      ? `${entry.category} · ${entry.channelAccount} → ${entry.targetAccount} · ${entry.note}`
      : `${entry.category} · ${entry.channelAccount} · ${entry.note}`;
  ...
  return {
    ...
    label: entry.note || entry.category,
    description,
    amount: `${amountPrefix}${formatMoney(entry.amount)}`,
...
function detectCategory(
  ...
  if (text.includes("咖啡") || text.includes("饭") || text.includes("外卖")) {
    return "餐饮";
  }
...
function ensureTransferTarget(value: string, options: string[], source: string) {
